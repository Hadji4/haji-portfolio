...
import { updateHeroPhoto } from "@/app/admin/(dashboard)/settings/actions";
...
export function HeroPhotoUploadForm({ currentPhotoUrl }: { currentPhotoUrl: string | null }) {
  const [state, formAction] = useActionState<ActionState, FormData>(updateHeroPhoto, {});
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          {displaySrc ? (
            ...
            <img src={displaySrc} alt="Hero" className="h-full w-full object-cover" />
          ) : (
      ...
      <div>
        <p className="font-medium text-foreground">Hero photo</p>
        <p className="text-sm text-muted">
          The portrait shown at the top of the public homepage. JPEG, PNG, WebP or GIF, up to 2MB.
        </p>
        <button
          type="button"
          onClick={() => inputRef.current?.click()}
